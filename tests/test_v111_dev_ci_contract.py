...
def test_v111_dev_ci_builds_without_publishing_and_marks_only_after_artifacts():
    text = WORKFLOW.read_text(encoding="utf-8")
    assert "- v1.11-dev" in text
    assert "v1.11-ci-green" in text
    assert "git push origin \"HEAD:refs/heads/v1.11-ci-green\" --force" in text

    artifact = text.index("- name: Upload Windows artifacts")
    marker = text.index("- name: Mark validated v1.11 development commit")
    ...
    assert "github.ref == 'refs/heads/v1.11-dev'" in marker_block
