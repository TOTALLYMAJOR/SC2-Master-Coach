(function(){
  ...
  const gameSecond=()=>{const text=document.getElementById("clock")?.textContent||"0:00",m=text.match(/(\d+):(\d+)/);return m?(+m[1]*60)+(+m[2]):0};
  ...
  function cast(s){
    ...
    set.add(s.key);lab.save(lab.KEYS.signals,[...set]);
    const times=lab.load(SIGNAL_TIME_KEY,{});times[s.key]={gameSecond:gameSecond(),at:Date.now()};lab.save(SIGNAL_TIME_KEY,times);
    speak(s.voice);lab.render();window.dispatchEvent(new CustomEvent("sc2:signal",{detail:{key:s.key,evidence:key,gameSecond:gameSecond()}}));
  }
  function clear(){
    document.querySelectorAll("#evidenceList [data-k].active").forEach(b=>b.click());
    lab.save(lab.KEYS.signals,[]);lab.save(SIGNAL_TIME_KEY,{});lab.render();
    window.dispatchEvent(new CustomEvent("sc2:signal",{detail:{clear:true,gameSecond:gameSecond()}}));
  }

  lab.register({
    id:"signals",label:"Quick Signals",
    render(el){
      const known=lab.knowledge(),set=signals(),plan=lab.activePlan(),times=lab.load(SIGNAL_TIME_KEY,{}),now=gameSecond();
      el.innerHTML=`<div class="lab-head"><div><b>Second-Screen Quick Signals</b><span>One tap supplies information the app cannot obtain live without game integration. Signals are time-stamped so Pro Mind can warn when a read becomes stale.</span></div><div class="lab-actions"><button class="hud-btn ${voiceOn()?"active":""}" id="quickVoice">Brief voice ${voiceOn()?"ON":"OFF"}</button><button class="hud-btn" id="compactLive">Compact live mode</button><button class="hud-btn" id="clearQuick">Clear</button></div></div><div class="opponent-report"><b>Opponent race</b>${["Unknown","Terran","Zerg","Protoss"].map(x=>`<button class="hud-btn ${known.opponent===x?"active":""}" data-opp="${x}">${x}</button>`).join("")}</div><div class="signal-grid">${SIGNALS.map(s=>{const age=times[s.key]?.gameSecond==null?"":` · ${Math.max(0,now-times[s.key].gameSecond)}s old`;return `<button class="spell-signal ${set.has(s.key)?"active":""}" data-signal="${s.key}"><b>${lab.safe(s.label)}</b><span>${lab.safe(s.voice)}${age}</span></button>`}).join("")}</div><div class="communication-rule"><b>Live communication governor</b><span>Normal callouts stay to one short sentence. The coach speaks scheduled build cues, explicit signals, and strategic overrides; explanations wait for replay review.</span><strong>${plan?`Active spell: ${lab.safe(plan.title)}`:"No spell loaded."}</strong></div>`;
      el.querySelectorAll("[data-signal]").forEach(b=>b.onclick=()=>{const s=SIGNALS.find(x=>x.key===b.dataset.signal);if(s)cast(s)});
      el.querySelectorAll("[data-opp]").forEach(b=>b.onclick=()=>{lab.setOpponent(b.dataset.opp);window.dispatchEvent(new CustomEvent("sc2:opponent",{detail:{opponent:b.dataset.opp}}))});
