(function(){
  ...
  function render(){
    ...
    root.innerHTML=`<div class="pro-head"><div><div class="section-title">PRO MIND // Unwritten Game <span>${SAFE(key)} · patch ${PATCH}</span></div><p>${SAFE(model.thesis)}</p></div><button class="hud-btn" id="proRead">Read Pro Mind</button></div>${response?`<div class="pro-override"><b>Evidence changed the thought process</b><strong>${SAFE(response.text)}</strong></div>`:""}<div class="pro-now"><div class="pro-question"><span>WHAT WOULD A PRO ASK RIGHT NOW?</span><strong>${SAFE(q?.question||"What information changes the next commitment?")}</strong><p>${SAFE(q?.why||"")}</p><small>UNSPOKEN RULE · ${SAFE(q?.rule||"Evidence outranks the script.")}</small></div><div class="pro-scout"><span>${scout&&t>=scout.from&&t<=scout.to?"SCOUT NOW":"NEXT INFORMATION WINDOW"}</span><strong>${SAFE(scout?.label||"Refresh information")}</strong><p><b>${SAFE(scout?.question||"What changed since the last read?")}</b><br>${SAFE(scout?.purpose||"Renew information before commitment.")}</p><small>${SAFE(scout?.changes||"The next decision should use fresh evidence.")}</small></div></div><div class="pro-section-title"><b>OPEN STRATEGIC BRANCHES</b><span>Do not choose a tech name; choose the capability you are missing.</span></div><div class="pro-branches">${branchHtml}</div><div class="pro-expansion"><div class="pro-expansion-head"><div><span>EXPANSION PERMISSION</span><b>${SAFE(model.expansion.name)}</b><p>${SAFE(model.expansion.thesis)}</p></div><div class="pro-permission ${exp.cls}"><strong>${exp.status}</strong><span>${exp.passCount}/${exp.total} gates</span></div></div><div class="pro-gates"><div class="pro-gate read ${exp.evidence}"><span>${exp.evidence==="pass"?"✓":exp.evidence==="fail"?"×":"?"}</span><b>Fresh threat / production read</b><small>${SAFE(infoLabel)}</small></div>${manualHtml}</div><div class="pro-expansion-note">The app can verify only reported evidence. Position, reinforcement access, and vision are manual confirmations because Master Coach has no live SC2 integration.</div></div><div class="pro-section-title"><b>YOU BOUGHT THIS — NOW PROTECT THE INVESTMENT</b><span>Major purchases create new responsibilities.</span></div><div class="pro-investments">${investments}</div>`;
    $("proRead").onclick=speakCurrent;root.querySelectorAll("[data-gate]").forEach(b=>b.onclick=()=>toggleGate(key,b.dataset.gate));
  }
  ...
  function mount(){
    ...
    const nodes=[$("clock"),$("railMatchup"),$("railRace"),$("planOpponent"),$("opponent")].filter(Boolean);const obs=new MutationObserver(render);nodes.forEach(n=>obs.observe(n,{childList:true,subtree:true,characterData:true,attributes:true}));
    ...
    const evidence=document.querySelector("#evidenceList");if(evidence)new MutationObserver(render).observe(evidence,{subtree:true,attributes:true,attributeFilter:["class"]});
  }
