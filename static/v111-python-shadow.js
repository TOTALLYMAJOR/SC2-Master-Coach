(function(){
  ...
  function ensureStatusChip(){
    ...
    if(!button){
      ...
      button.title="Python Intelligence runs in Shadow Mode on v1.11.0. It may calculate a second opinion but cannot mutate the live Strategic OS plan.";
      host.prepend(button);
    }
    const row=scienceState();button.textContent=row.label;button.className=`v110-btn v111-science-chip ${row.cls}`;
  }
