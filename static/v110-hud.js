(function(){
  ...
  async function synchronize(){const box=$("v110Countdown"),number=$("v110CountNumber");if(!box||!number)return;box.hidden=false;for(const value of ["3","2","1","LIVE"]){number.textContent=value;playEarcon(value==="LIVE"?"good":"info");await wait(650)}box.hidden=true;timerSeconds=0;timerSynced=true;timerRunning=true;timerAnchor=performance.now();if(mode==="1v1"){try{E.start();E.setClock(0);E.evaluate()}catch(_e){}}startTimer();view="hud";render()}
  function wait(ms){return new Promise(resolve=>setTimeout(resolve,ms))}
  function stopTimer(){if(timerHandle){cancelAnimationFrame(timerHandle);timerHandle=null}}
  function startTimer(){stopTimer();timerAnchor=performance.now();const tick=now=>{if(timerRunning){const delta=Math.max(0,Math.min((now-timerAnchor)/1000,.25));timerSeconds+=delta;timerAnchor=now;const whole=Math.floor(timerSeconds);if(mode==="1v1"){try{E.setClock(whole);if(whole%5===0)E.evaluate()}catch(_e){}}updateHudDynamic();maybeBuildPrep()}else timerAnchor=now;timerHandle=requestAnimationFrame(tick)};timerHandle=requestAnimationFrame(tick)}
  ...
  function shiftTimer(delta){timerSeconds=Math.max(0,timerSeconds+delta);timerSynced=false;timerAnchor=performance.now();if(mode==="1v1"){try{E.setClock(Math.floor(timerSeconds));E.evaluate()}catch(_e){}}render();toast(`Timer ${delta>=0?"+":""}${delta}s · now approximate`)}
